...
public:
...
#else
	pthread_t hThread_;
	pthread_t hGUIThread_;
	pthread_cond_t hEvent_;
	pthread_cond_t hEvent2_;
	pthread_cond_t hDynEvent_;
	...
	Pixmap hThinking_;
	Pixmap hDead_;
	Pixmap hHungryLeftFork_;
	Pixmap hHungryRightFork_;
	Pixmap hHungryBothFork_;
...
namespace ObserverImpl
{
class CCM_Observer_impl : virtual public CCM_Observer_skel,
			  public Components::SessionComponent
{
    CCM_Observer_impl(const CCM_Observer_impl&);
    void operator=(const CCM_Observer_impl&);   
    
    CCM_Observer_Context_var ctx_;
...
public:
    ...
    virtual void push_philosopher_state(Dinner::PhilosopherState* event)
        throw(CORBA::SystemException);

    virtual void set_session_context(::Components::SessionContext_ptr ctx)
    {
    	ctx_ = CCM_Observer_Context::_narrow(ctx);
    }
	...
	friend unsigned int ::ObGUIThreadProc(Dinner::ObserverImpl::CCM_Observer_impl* this_);
};

class CCM_ObserverHome_impl : virtual public CCM_ObserverHome_skel
{
