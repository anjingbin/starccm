...
label .frame2.hostipAddressLable -text "Please input the host's ip:"
entry .frame2.hostipAddressEntry -width 40
.frame2.hostipAddressEntry insert 0 $hostip
pack .frame2.hostipAddressLable .frame2.hostipAddressEntry -side left

#button .deployButton  -text "Start Deploy" -width 80 \
#    -command "startDeploy"
#pack .deployButton -side top

button .hostInstallButton  -text "Start HostInstall" -width 80 \
    -command "startHostInstall"
pack .hostInstallButton -side top

button .stopButton  -text "Stop HostInstall" -width 80 \
    -command "stopHostInstall"
...
proc startDeploy {} {
	setDefaultValue
	exec Deploy.exe < demo.ini &
}

proc startHostInstall {} {
	setDefaultValue
	exec HostInstall.exe < host.ini &
}

proc stopHostInstall {} {
	setDefaultValue
	catch {exec ps.exe | grep.exe ComponentServer > pid.txt}
	catch {exec ps.exe | grep.exe ServerActivator >> pid.txt}
	catch {exec ps.exe | grep.exe HostInstall >> pid.txt}
...
proc setDefaultValue {} {
	set fd [open "host.ini" w]
	fconfigure $fd -buffering line
	set ip [ .frame1.ipAddressEntry get ]
	puts $fd $ip
	set hostip [ .frame2.hostipAddressEntry get ]
	puts $fd $hostip
	close $fd    
}
