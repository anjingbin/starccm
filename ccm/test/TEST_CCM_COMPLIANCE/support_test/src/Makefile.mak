...
DLLS = StdPlusDll PlusDll support_client
...
install:: all
	if exist $(top_srcdir)\bin\support.zip \
		$(top_srcdir)\bin\unzip.exe -o -d temp $(top_srcdir)\bin\support.zip
	...
	cd temp & \
	$(top_srcdir)\..\bin\zip.exe -r support.zip * & \
	move support.zip $(bindir)
	$(top_srcdir)\bin\rm -rf temp
	
	copy ..\bin\support_client.exe $(bindir)
