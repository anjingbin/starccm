...
DLLS = StdTestDll TestDll session_client
...
install:: all
	if exist $(top_srcdir)\bin\session.zip \
		$(top_srcdir)\bin\unzip.exe -o -d temp $(top_srcdir)\bin\session.zip
	...
	cd temp & \
	$(top_srcdir)\..\bin\zip.exe -r session.zip * & \
	move session.zip $(bindir)
	$(top_srcdir)\bin\rm -rf temp
	
	copy ..\bin\session_client.exe $(bindir)
