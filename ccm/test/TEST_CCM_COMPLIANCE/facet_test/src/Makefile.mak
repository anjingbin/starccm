...
DLLS = StdTestDll TestDll facet_client
...
install:: all
	if exist $(top_srcdir)\bin\facet.zip \
		$(top_srcdir)\bin\unzip.exe -o -d temp $(top_srcdir)\bin\facet.zip
	...
	cd temp & \
	$(top_srcdir)\..\bin\zip.exe -r facet.zip * & \
	move facet.zip $(bindir)
	$(top_srcdir)\bin\rm -rf temp
	
	copy ..\bin\facet_client.exe $(bindir)
