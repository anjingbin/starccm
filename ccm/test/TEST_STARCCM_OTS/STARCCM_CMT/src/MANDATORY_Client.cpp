...
{
	ofstream f("MANDATORY_Client.txt",ios_base::out | ios_base::app );
	if(!f) 
	{
...
run(CORBA::ORB_ptr orb, int argc, char* argv[])
{
    ...
    //file init
    ofstream f("MANDATORY_Client.txt");
    f.close();
    ...
    do
    {
		...
		if(c == 'y')
		{
			...
			{
				cout << "The mandatory policy raises CORBA::TRANSACTION_REQUIRED execption." << endl;			
				slog("The mandatory policy raises CORBA::TRANSACTION_REQUIRED execption.\n");
			}
