...
{
	ofstream f("NEVER_Client.txt",ios_base::out | ios_base::app );
	if(!f) 
	{
...
run(CORBA::ORB_ptr orb, int argc, char* argv[])
{
    ...
    do
    {
		...
		if(c == 'y')
		{
			...
			{
				cout << "The never policy raises CORBA::INVALID_TRANSACTION execption." << endl;			
				slog("The never policy raises CORBA::INVALID_TRANSACTION execption.\n");
			}
