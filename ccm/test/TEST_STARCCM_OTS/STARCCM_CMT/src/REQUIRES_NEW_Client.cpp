...
{
	ofstream f("REQUIRES_NEW_Client.txt",ios_base::out | ios_base::app );
	if(!f) 
	{
...
run(CORBA::ORB_ptr orb, int argc, char* argv[])
{
    ...
    //file init
    ofstream f("REQUIRES_NEW_Client.txt");
    f.close();
    ...
    do
    {
		...
		if(c == 'y')
		{
			...
			{
				cout << "The requires_new policy raises other exeception." << endl;
				slog("The requires_new policy raises other exeception.\n");
			}
