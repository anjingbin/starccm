/*this file come from RFC, for generating UUID*/
#include <string.h>
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
static void write_state(unsigned16 clockseq, uuid_time_t timestamp,
						uuid_node_t node);
static void format_uuid_v1(uuidtype * uuid, unsigned16 clockseq,
						   uuid_time_t timestamp, uuid_node_t node);
static void format_uuid_v3(uuidtype * uuid, unsigned char hash[16]);
...
and node ID */
void format_uuid_v1(uuidtype * uuid, unsigned16 clock_seq, uuid_time_t
...
void uuid_create_from_name(
						   uuidtype * uuid,        /* resulting UUID */
												 uuidtype nsid,          /* UUID to serve as context, so identical
    ...
    uuidtype net_nsid;      /* context UUID in network byte order */
    ...
    MD5Init(&c);
    MD5Update(&c, &net_nsid, sizeof(uuidtype));
...
*/
void format_uuid_v3(uuidtype * uuid, unsigned char hash[16]) {
/* Construct a version 3 uuid with the (pseudo-)random number
	* plus a few constants. */
	
	memcpy(uuid, hash, sizeof(uuidtype));
