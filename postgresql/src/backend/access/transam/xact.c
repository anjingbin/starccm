 ...
 *
 * IDENTIFICATION
 *	  $Header: /home/hjcvs/OB-CCM-1.0/postgresql/src/backend/access/transam/xact.c,v 1.2 2004/07/05 08:55:22 SuLiang Exp $
 *
 * NOTES
...
XaCommitTransaction()
{
	 ...
	 */
	 ...
	 //return;
	 if (s->state != TRANS_PREPARE)
	 {
		 elog(NOTICE, "CommitTransaction should in prepare state ");
		 return;
	 }
     ...
     /*      We have finish them in prepare phase
      *      if we commit transactin in 2PC --Wall
      */
      ...
      s->state = TRANS_COMMIT;
      /* delrecord from locklog */
     
      //DelRecordOfLockLog(s->transactionIdData); //renyi 10 25:changed 
...
XaAbortTransaction()
{
    ...
    if (s->state !=TRANS_PREPARE)
    {
          /* We abort the transaction when the transaction is in-progress or pre
pared after vote commit. --Wall 05/19/99  */
                elog(NOTICE, "XaAbortTransaction should in prepare state ");
                return;
    }
...
//renyi 10 25
 ...
 /*
  *     PrepareTransaction will pesuo-commit the transaction,
  *     we will recored the transaction PREPARED in the log.
  *     So the system don't think this transaction commited.
  *     we don't release the locks hold by the transaction until
  *     the true commit or rollback.    --Wall 05/25/99
  */
...
static int
 ...
 //int                    result;
        ...
        //if (s->state == TRANS_DISABLED)         return XAABORT ;
...
StartTransactionCommand(void)
{
	...
	switch (s->blockState)
	{
		...
		case TBLOCK_BEGIN:
			 ...
			 */
	    case TBLOCK_PREPARE:
                        /* we should accept rollback,end,abort,commit ,but now
                         * I don't implement  -renyi 10 25*/
		...
		//renyi 10 25
		case TBLOCK_XAEND: 
            elog(NOTICE, "StartTransactionCommand: unexpected TBLOCK_XAEND");
                        s->blockState = TBLOCK_DEFAULT;
                        XaCommitTransaction();
                        StartTransaction();
            break;
     	...
     	case TBLOCK_XAABORT:
            elog(NOTICE, "StartTransactionCommand: unexpected TBLOCK_XAABORT");
            break;
	}
...
CommitTransactionCommand(void)
{
	...
	switch (s->blockState)
	{
		...
		case TBLOCK_PREPARE:	//renyi 11 14
		case TBLOCK_INPROGRESS:
			CommandCounterIncrement();
			MemoryContextResetAndDeleteChildren(TransactionCommandContext);
			break;
...
void XaCommitTransactionBlock(int normal)
{
        TransactionState s = CurrentTransactionState;
        /* ----------------
         *      check the current transaction state
         ...
         */
         ...
         if (normal==XARECOVERY)
              {

                 //ReCommitTransaction();  //renyi 10 25:changed
                 return;
              }
        ...
        if ( s->blockState==TBLOCK_PREPARE)
        {
                 ...
                 *      will recognize this and commit the transaction and retur
n
                 *      us to the default state
                 * ----------------
                 */
                //XaCommitTransaction(); /*  --it will be called in other places*/
                s->blockState = TBLOCK_XAEND;
                return;
        }
        ...
        /* If we xacommit the transaction without prepare ,we will do it as
endtransactionblock. */
     ...
     * ----------------
     */
    elog(NOTICE, "EndTransactionBlock and not inprogress/abort/prepare state.\n ");
    s->blockState = TBLOCK_ENDABORT;
        return;
}

int
 XaPrepareTransactionBlock()
{
        TransactionState s = CurrentTransactionState;
        ...
        /* before prepare vote commit, anything will be done,no abort and no com
mit. if we vote no, abort the current transaction. --Wall 4-13-99 */

        if (s->blockState!=TBLOCK_INPROGRESS)
        {


        /*      AbortCurrentTransaction(); may be have some question. */
        /*  only let the forntend get return value by inbuffer .at best, we can
return value.*/
                 AbortTransaction();
                 s->blockState = TBLOCK_ENDABORT;
        /* because we find the transaction isn't in progress,we do nothing */
         ...
         /* here we nedd  do somehing such as recored some information about the
 prepared transaction into log including lock .  ---Wall 4-13-1999 */

       switch(PrepareTransaction()){
        case XAABORT :
                 AbortTransaction();
                 s->blockState = TBLOCK_ENDABORT;
               /* elog(NOTICE,"VOTE ROLLBACK .\n");
                break; */
		return XAABORT;
        case XACOMMIT:
        s->blockState=TBLOCK_PREPARE;
   /* elog(NOTICE,"VOTE COMMIT.the transactionid is %d\n",GetCurrentTransactionId());
        ...
        case XAREADONLY:
                 CommitTransaction();
                 s->blockState = TBLOCK_ENDABORT;/* I think it should be END */

/*               elog(NOTICE,"VOTE READONLY.\n");
                break;*/
	 	return XAREADONLY;
...
void XaRollbackTransactionBlock(int normal)
{
        ...
        /*
         * if the transaction has already been not prepared ,it may mean
         * this transaction does not support XA. If you want abort it, you shoul
d

       * type rollback without  xa_. --wall 10-25-99
         */
         ...
         switch (normal)
                 ...
                 case XANORMAL:
                {
                        ...
                        if (s->blockState==TBLOCK_PREPARE)
                                {
                                /* ----------------
                                *       here we were inside a transaction block
and we
                                *       got an abort command from the user, so w
e move to
                                *       the abort state, do the abort processing
 and
                                *       then change to the ENDABORT state so we
will end up
        ...
        elog(NOTICE, "XaAbortTransactionBlock and not inprogress/prepare state");
    AbortTransaction();
    s->blockState = TBLOCK_ENDABORT;
