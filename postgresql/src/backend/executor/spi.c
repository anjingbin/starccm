 ...
 *
 * IDENTIFICATION
 *	  $Header: /home/hjcvs/OB-CCM-1.0/postgresql/src/backend/executor/spi.c,v 1.2 2004/07/05 08:55:28 SuLiang Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
_SPI_execute(char *src, int tcount, _SPI_plan *plan)
{
	...
	foreach(queryTree_list_item, queryTree_list)
	{
		...
		if (queryTree->commandType == CMD_UTILITY)
		{
			...
			{
				int i=2;//renyi 11 24
				ProcessUtility(queryTree->utilityStmt, None, NULL,&i);//renyi 11 24
				if (!islastquery)
					CommandCounterIncrement();
				else
					return res;
