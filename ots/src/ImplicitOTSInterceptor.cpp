...
public:
    ...
    pre_init(PortableInterceptor::ORBInitInfo_ptr info)
    {
		...
		piCurrent, slotId, cfgDefaultTimeout);//renyi 6 9
    ...
    post_init(PortableInterceptor::ORBInitInfo_ptr info)
    {
		...
		OBPortableInterceptor::ORBInitInfo_var starinfo =
		    OBPortableInterceptor::ORBInitInfo::_narrow(info);
...
//
void
OB::OTSInit(int& argc, char** argv)
{
    #ifdef _DEBUG_
		cout << "[OB::OTSInit begin!]" << endl;
	#endif

	assert(!OB::OTSInitCalled);
    OB::OTSInitCalled = true;
