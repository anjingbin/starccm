...
{
	cout << "test synchronzation resource server side program is running..." << endl;
    //
    // Resolve Root POA
    //
    ...
    //
    // The counter requires a transaction be active
    //
    ...
    //
    Counter_impl* counterImpl = new Counter_impl(orb, txnPOA, current);
    PortableServer::ServantBase_var servant = counterImpl;
    Counter_var counter = counterImpl -> _this();
    
    //
    // Save reference
    //
    CORBA::String_var s = orb -> object_to_string(counter);
    
    const char* refFile = "Counter.ref";
